...
export default function PptCompressorClient() {
  ...
  const [downloadName, setDownloadName] = useState("output.pptx");
  ...
  const compress = async () => {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : "压缩失败，请确认文件为有效的 .pptx。");
    } finally {
      setIsWorking(false);
    }
  };
  ...
  return (
    <ToolPageLayout toolSlug="ppt-compressor">
          ...
          >
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div className="text-sm text-slate-700">
                说明：这是“轻量压缩”（重新打包 PPTX 的 ZIP 容器），不解析/重压图片与媒体，体积不一定变小。
              </div>
              <button
                type="button"
              ...
              >
                {file ? "替换 .pptx" : "选择 .pptx"}
              </button>
              <input ref={inputRef} type="file" accept=".pptx" className="hidden" onChange={onChange} />
            </div>
            <div className="mt-2 text-[11px] text-slate-500">支持点击上传与拖拽上传 PPTX，拖拽可直接替换当前文件。</div>
          </div>

          {!file && (
            <div className="mt-6 rounded-3xl bg-slate-50 p-6 ring-1 ring-slate-200 text-sm text-slate-700">
              选择一个 PPTX 文件后，可调整压缩等级并导出。
            </div>
          )}
            ...
            <div className="mt-6 grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
              ...
              <div className="rounded-3xl bg-slate-50 p-5 ring-1 ring-slate-200 text-sm text-slate-700">
                ...
                <ul className="mt-3 list-disc space-y-2 pl-5">
                  <li>若体积没有变小，通常是因为资源本身已高度压缩（图片/视频）。</li>
                  <li>想要明显减小体积，通常需要压缩图片、降低分辨率或移除媒体资源。</li>
