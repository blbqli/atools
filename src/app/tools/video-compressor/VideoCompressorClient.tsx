...
const DEFAULT_UI: Ui = {
  hint: "视频压缩器：输出 MP4(H.264) 并用 CRF 控制体积/质量，可选 720p/1080p 缩放（纯前端本地处理不上传）。首次需加载 ffmpeg.wasm。",
  pick: "选择视频文件",
  replace: "点击替换视频",
  clear: "清空",
  ...
  errCompressFailed: "压缩失败（可能输出编码器未内置或浏览器资源不足）。",
...
export default function VideoCompressorClient() {
  return (
    <ToolPageLayout toolSlug="video-compressor" maxWidthClassName="max-w-6xl">
...
function Inner({ ui }: { ui: Ui }) {
  ...
  const [downloadName, setDownloadName] = useState<string>("compressed.mp4");
  ...
  const pickFile = (selected: File) => {
    ...
    const base = selected.name.replace(/\.[^.]+$/u, "") || "video";
    setDownloadName(`${base}.compressed.mp4`);
  };
  ...
  const clear = () => {
    ...
    setDownloadName("compressed.mp4");
  };
  ...
  const compress = async () => {
    ...
    const safeCrf = clampInt(crf, 18, 35);
    ...
    args.push("-c:v", "libx264", "-preset", preset, "-crf", String(safeCrf), "-pix_fmt", "yuv420p", "-c:a", "aac", "-b:a", `${safeAb}k`, outName);
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : ui.errCompressFailed);
    } finally {
      setIsWorking(false);
    }
  };
        ...
        <div className="mt-5 grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
          <div className="rounded-3xl bg-white p-5 ring-1 ring-slate-200">
            ...
            <div className="mt-5 grid gap-3">
              ...
              <label className="block text-sm text-slate-700">
                {ui.crf} <span className="text-xs text-slate-500">(18-35)</span>
                <input type="number" min={18} max={35} value={crf} onChange={(e) => setCrf(clampInt(Number(e.target.value), 18, 35))} className="mt-2 w-full rounded-2xl border border-slate-200 bg-white px-4 py-2 text-sm outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-400/30" />
              </label>
              <label className="block text-sm text-slate-700">
                {ui.preset}
            ...
            <div className="mt-4 flex flex-wrap items-center gap-2">
              <button type="button" onClick={() => void compress()} disabled={ffmpegState !== "ready" || isWorking} className="rounded-2xl bg-emerald-600 px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-emerald-700 disabled:opacity-60">
